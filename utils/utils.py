...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres): # 输入是预测框，预测类别置信度，标签，处理后的先验框，标签，阈值。
    # 输出是IOU得分值，预测值和真实值直接的差异，含有物体的网格，没有物体的网格，处理后的标签的x,y,w,h，真实框的置信度
    ...
    tcls = FloatTensor(nB, nA, nG, nG, nC).fill_(0) # 4×3×13×13中数据80个类别中的哪个类

    # Convert to position relative to box
    target_boxes = target[:, 2:6] * nG # target中的xywh都是0-1的，把整张图像看做1，可以得到其在当前gridsize上的xywh，映射到格子中
    gxy = target_boxes[:, :2]
    gwh = target_boxes[:, 2:]
    # Get anchors with best iou  每个格子中都有三种候选框
    ious = torch.stack([bbox_wh_iou(anchor, gwh) for anchor in anchors]) # 每一种规格的anchor跟每个标签上的框的IOU得分
    print (ious.shape)
    best_ious, best_n = ious.max(0) # 得到其最高分以及哪种规格框和当前目标最相似，best_n：0,1,2每种候选框标号
    # Separate target values
    b, target_labels = target[:, :2].long().t() # 真实框所对应的batch，以及每个框所代表的实际类别
    gx, gy = gxy.t()
    gw, gh = gwh.t()
    gi, gj = gxy.long().t() # 当前坐标对应的格子的位置，位置信息，向下取整了
    # Set masks
    ...
    tcls[b, best_n, gj, gi, target_labels] = 1 # 将真实框的标签转换为one-hot编码形式，80个类别是哪个类别就给哪个类别标1
    # Compute label correctness and iou at best anchor 计算预测的和真实一样的索引
    class_mask[b, best_n, gj, gi] = (pred_cls[b, best_n, gj, gi].argmax(-1) == target_labels).float()  # 预测值和真实值之间是计算对了还是错了
